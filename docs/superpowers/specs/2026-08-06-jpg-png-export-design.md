...
2. **가드** — 그럼에도 넘는 경우(네트워크 경로 등)를 위해 `ensure_writable_path(path)`를 두고,
   출력 경로가 확정되는 세 지점(단일/분할/배치)에서 **쓰기 전에** 부른다. 한계·실제 길이·구체적
   해결책(출력 폴더를 짧게 / 접미사를 짧게 / 분할 끄기)을 담은 에러를 낸다.
