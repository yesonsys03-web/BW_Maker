...
- 결과물은 항상 **새 PSD 파일** (원본 절대 수정 안 함). 출력 파일명은 접미사 규칙(기본 `_LINE`).
...
출력 파일은 항상 **원본과 같은 폴더**에 `<원본이름><outputSuffix>.psd`로 생성한다.
동명 파일이 이미 있으면 덮어쓰기 전에 확인(배치에서는 파일별 확인 대신 실행 전 일괄 확인).
