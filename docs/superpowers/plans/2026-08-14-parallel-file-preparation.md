...
병렬은 `scripts/prepare-parallel.py`, 같은 폴더·같은 프리셋·같은 콜드 조건. 프런트와 같은
당겨 가기 규율로 워커를 돌린다.

| 조건 | 파일 수 | 벽시계 | 배율 |
|---|---|---|---|
| **순차 (기준선, 콜드)** | 100 | **28.0분** (1,679초) | 1.00 |
| **작업 프로세스 2** | 100 | **14.6분** (878초) | **1.91** |
| **작업 프로세스 4** | 100 | **8.6분** (513초) | **3.27** |
| 순차 — 가장 무거운 판 한 장 | 1 | 4.4분 (262초) | 1.00 |
| 작업 프로세스 4 — 같은 한 장 | 1 | 4.5분 (267초) | **0.98** |

작업 프로세스 4개의 파일 분배: `[25, 35, 8, 32]`. 8장만 받은 프로세스가 무거운 판들을
물었다 — 미리 나누지 않고 당겨 가게 한 이유가 이 숫자다. 실패 0장.

**한 장짜리는 빨라지지 않는다 — 오히려 2% 느리다.** 병렬은 **파일 단위**로만 나누므로
한 장은 여전히 프로세스 하나가 통째로 처리하고, 워커를 띄우는 비용만 더 든다. 이 트랙이
고치는 것은 "폴더를 열고 기다리는 시간"이지 "무거운 판 한 장"이 아니다.

**이론 하한과의 거리.** 4개의 하한은 `max(1679/4, 262) = 420초`였고 실측은 513초 —
하한의 82%다. 남은 22%는 꼬리에서 온다: 마지막 무거운 판을 도는 동안 다른 프로세스가
놀기 때문이고, 파일 단위 분할인 한 없앨 수 없다.

**작업 프로세스를 8개로 늘려도 7분 아래로 못 간다** — 가장 무거운 판 한 장이 262초라
`1679/8 = 210초 < 262초`가 되어 그 판 하나가 벽시계를 정한다. 더 줄이려면 **판 하나
안에서**(뷰·잎 단위) 나눠야 하고, 그건 다른 설계다.
