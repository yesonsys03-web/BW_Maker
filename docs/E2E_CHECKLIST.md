...
대상 파일(둘 다 존재/크기 확인함). 실제 파일명은 공개 저장소용으로 익명화했다 —
로컬에서 동등한 규모의 실제 BG PSD 두 개로 바꿔 진행하면 된다. 두 번째 파일명에
들어간 공백은 의도적이다(경로에 공백이 있는 케이스를 같이 태운다):
...
3. 첫 번째 파일(430MB)을 클릭해 연다 — "처리중" → "열림"으로 바뀌고 레이어 트리/썸네일이
   ...
   에러 없이 전체가 로드돼야 함).
4. 상단 프리셋 바에서 'line 추출' 프리셋(또는 동등한 이름 규칙 프리셋)을 선택 후 "적용" —
   매칭된 레이어가 강조되고, 매칭 결과가 라인 레이어만으로 채워지는지(색 지정용 `line col`류나
   `LINEAR DODGE`류가 섞여 있지 않은지) 확인. 매칭 규칙이 바뀔 때마다 정확한 건수를 실측하려면
   `scripts/audit-line-matching.py <폴더>`로 실제 납품 폴더에 규칙 변경 전/후를 돌려 비교한다
   (하드코딩된 건수는 규칙이 바뀔 때마다 낡으므로 여기 적지 않는다).
