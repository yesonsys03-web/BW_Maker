...
import { defaultExportPath, findTreeName, reorderArgs, resolveEntryName } from "./exportFlow";
import type { Entry } from "./opsReducer";
import type { TreeNode } from "./types";

// ---------- defaultExportPath ----------

test("defaultExportPath appends suffix before extension, forces .psd", () => {
  expect(defaultExportPath("/a/b/name.psd", "_LINE")).toBe("/a/b/name_LINE.psd");
});

test("defaultExportPath replaces a non-psd extension with .psd", () => {
  expect(defaultExportPath("/a/b/name.PSD", "_LINE")).toBe("/a/b/name_LINE.psd");
});
...
test("defaultExportPath handles a dotfile-style name with no extension", () => {
