...
test("with edge lines off, different numeric settings still get the same key", () => {
  const edgeOffA: EdgeLines = { enabled: false, threshold: 10, gap: 4, width: 5, minLength: 8, lineAlpha: 64, colourMode: "composite", edgeMode: "region", widthScale: 1 };
  const edgeOffB: EdgeLines = { enabled: false, threshold: 90, gap: 40, width: 50, minLength: 80, lineAlpha: 6, colourMode: "composite", edgeMode: "region", widthScale: 1 };
