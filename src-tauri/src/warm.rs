...
/// 워커 하나에 파일 하나를 먹인다. 워커가 이미 죽었으면 에러 — 프런트는 그
/// 파일을 다른 워커에 다시 준다.
#[tauri::command]
pub fn warm_worker_send(state: State<'_, WarmState>, id: u32, path: String) -> Result<(), String> {
    let mut workers = state.workers.lock().unwrap();
    let proc = workers.get_mut(&id).ok_or("no such worker")?;
    let line = json!({ "path": path }).to_string();
