...
#: 축소한 획의 알파에 거는 배수. **굵히지 않고 진하게만** 한다.
#:
#: 굵기는 정상 축소(블록 평균) 그대로 두고 알파만 곱한다. 최댓값으로 채우면
#: 획이 블록 경계에 걸릴 때 양쪽 블록이 다 꽉 차서 1px 획이 2px 진한 획이
#: 되는데, 아티스트가 본 것이 정확히 그것이다("굵으니까 진해 보이지, 딱 봐도
#: 2배", 2026-08-13). 평균은 두 블록에 반씩 나눠 담아 안티에일리어싱된 한 줄로
#: 남으므로 굵기가 원본 비율을 지킨다.
#:
#: 4배인 이유: 배율 0.128에서 2px 획의 평균 알파가 ~32라 그대로면 안 보이고
#: ("생성이 안 됐다" 신고 두 건), 4배면 ~128로 확실히 보이면서 255로 뭉개지지
#: 않는다. 이미 진한 획은 255에서 잘리는데, 그건 원래 진한 획이라 맞는 결과다.
OVERLAY_ALPHA_GAIN = 4.0
...
def _visible_reduce(arr, k):
    ...
    한 번의 reshape에서 최댓값과 평균을 함께 뽑는다:
      ...
      알파 — **평균에 배수만** 건다(OVERLAY_ALPHA_GAIN). 평균이 곧 정상 축소라
             굵기는 원본 비율 그대로고, 배수는 그 profile을 통째로 올릴 뿐이라
             획이 두꺼워지지 않는다.

    알파를 최댓값으로 채우면 안 된다. 획이 블록 경계에 걸릴 때 양쪽 블록이 다
    꽉 차서 1px 획이 2px 진한 획이 되고, 아티스트는 그것을 "생성된 라인이
    이상하다"로 읽는다 — 1cc4b4f와 이 함수의 첫 판이 그렇게 신고됐다. 반대로
    평균만 쓰면 몇 px짜리 획이 20% 알파 안개가 되어 안 보인다.
    """
    h, w = arr.shape[:2]
    ph, pw = (-h) % k, (-w) % k
    if ph or pw:
    ...
    gained = blocks[..., 3].mean(axis=(1, 3)) * OVERLAY_ALPHA_GAIN
    # 원래 알파(최댓값)를 넘겨 칠하지 않는다 — 반투명 획이 불투명해지면 그것도
    # 없는 그림이다.
    out[..., 3] = np.minimum(gained, out[..., 3]).round().astype(arr.dtype)
    return out
