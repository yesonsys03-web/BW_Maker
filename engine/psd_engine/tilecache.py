...
#: 그룹 썸네일 캐시의 형식 판. 그룹 합성(타일 경로 포함)이 바뀌어 같은 입력에서
#: 다른 그림이 나오게 되면 올린다 — OVERLAY_FORMAT과 같은 규율.
GROUP_THUMB_FORMAT = 1


def group_thumb_key(group_id, max_size, visible_ids):
    """
    그룹 썸네일 캐시 파일명 조각. 보이는 자손 id 집합이 키에 들어간다 — 썸네일은
    자식 가시성을 반영하므로(render_thumbnails), 눈을 바꾸면 키가 갈려 자동으로
    다시 그려진다. 그림 자체를 기억할 뿐 합성 방식은 건드리지 않으므로 블렌드·
    마스크가 걸린 그룹에서도 픽셀이 1도 안 바뀐다.
    """
    raw = json.dumps(
        [GROUP_THUMB_FORMAT, group_id, max_size, sorted(visible_ids)],
        separators=(",", ":"))
    return hashlib.sha1(raw.encode("utf-8")).hexdigest()[:16]


def _group_thumb_path(path, mtime, key):
    return _file_dir(path, mtime) / f"g{key}.png"


def load_group_thumb(session, key):
    """디스크에서 그룹 썸네일(작은 RGBA PNG)을 읽는다. 미스·손상·꺼짐이면 None."""
    skey = _session_key(session)
    if not ENABLED or skey is None:
        return None
    f = _group_thumb_path(skey[0], skey[1], key)
    try:
        img = Image.open(f)
        img.load()
...
def store_group_thumb(session, key, img):
    """그룹 썸네일(이미 max_size로 줄인 것)을 디스크에 떨군다. 타일과 같은 원자 쓰기."""
    skey = _session_key(session)
    if not ENABLED or skey is None:
        return
    d = _file_dir(skey[0], skey[1])
    try:
        ...
        try:
            ...
            os.replace(tmp, _group_thumb_path(skey[0], skey[1], key))
        except BaseException:
            try:
                os.unlink(tmp)
            except OSError:
